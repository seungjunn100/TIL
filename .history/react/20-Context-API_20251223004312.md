...
- 빈 객체인 `CounterContext`에 데이터(`value`)를 담아서 `Context.Provider` 컴포넌트를 생성한다.
  
  - 자식과 하위의 모든 자손 컴포넌트에서 값을 사용할 수 있게된다.
...
- `CounterProvider` 컴포넌트에 의해 새로운 `Provider` 컴포넌트가 만들어지며, `Context`를 제공받아 모든 자식 컴포넌트에서 사용할 수 있다.
...
- 모든 자식 컴포넌트에서 사용 가능하므로, 최하위 컴포넌트에서도 사용이 가능하다.

- `useContext` 훅을 사용하면 `context`를 반환 받는다.
