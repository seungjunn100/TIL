...
- [예제로 보는 라우팅 동작 구현](#예제로-보는-라우팅-동작-구현)
  - [전통적인 웹 페이지 이동](#전통적인-웹-페이지-이동)
  - [리액트(SPA) 웹 페이지 이동](#리액트spa-웹-페이지-이동)
...
- 최초 마운트시에 `useEffect`에 의해 `popstate` 이벤트가 발생하면 `handleLocationChange` 함수가 동작하도록 브라우저에 등록해 두었다.

- 위에서 링크를 클릭하여 URL이 변경되고 `popstate` 이벤트가 동작하게 된다.

- `handleLocationChange` 함수가 실행되고 상태값이 새로 바뀌었기 때문에 리렌더링이 이루어 진다.

  - 새로운 상태 값에 따른 컴포넌트로 리렌더링이 이루어 진다.