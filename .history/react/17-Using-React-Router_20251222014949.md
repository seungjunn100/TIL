...
- [리액트 라우터가 제공하는 주요 훅](#리액트-라우터가-제공하는-주요-훅)
...
- 어떤 라우트와도 매칭되지 않을 때, 사용자가 잘못된 `URL`로 접근했을 때 보여주는 화면

![404 라우트 (Not Found)](../src/images/not-found.png)

- `catch-all` 라우트
  
  - 위의 라우트와 일치하지 않은 모든 `URL`은 `NotFound` 렌더링

#### errorElement (라우트 단위 에러 처리)

- 해당 라우트가 실행되는 도중 에러가 발생했을 때 보여주는 화면

- 주로 `Data Router`에서 사용한다.

- `loader`에서 에러 발생, `action` 처리 중 에러 발생, 렌더링 중 `throw`된 에러 상황에서 해당 라우트의 `errorElement`가 렌더링된다.

![errorElement (라우트 단위 에러 처리)](../src/images/errorElement.png)

- `Todo` 라우트에서 발생하는 에러에 대한 처리
