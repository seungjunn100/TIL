...
- `Context`를 제공받기 위해서는 `Provider`안에서만 사용할 수 있기 때문에, 컴포넌트 재활용이 어렵다.

  ![Context Api Weakness - 1](../src/images/context-api-weakness-1.png)

- 여러 상태를 관리하기 위해 객체를 상태로 지정할 수 있지만, 속성중 하나만 변경되어도 구독하고 있는 모든 컴포넌트가 리렌더링된다.

  ![Context Api Weakness - 2](../src/images/context-api-weakness-2.png)

  - 만약 테마 상태를 관리하기 위해 추가하고, 테마 속성을 변경하게되면 카운트 관련 속성을 가진 컴포넌트들도 같이 리렌더링이 발생하게 된다.

  - `Context`를 여러 개로 분리해서 사용하는게 좋다.

- `Provider`의 `Context`를 전달 받아 사용하는 컴포넌트에 의해 상태가 변경 되면, `Context`를 구독하고 있는 다른 컴포넌트들도 리렌더링된다.

  ![Context Api Weakness - 3](../src/images/context-api-weakness-3.png)

  - `useState`에 의해 상태값이 변경되면 변경된 상태값으로 컴포넌트가 리렌더링된다.
  
  - 불필요한 리렌더링을 방지하려면 `useMemo`, `useCallback`을 사용하여 메모이제이션하는 것이 좋다.