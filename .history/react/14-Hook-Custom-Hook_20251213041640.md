...
- 타입 설정 각 컴포넌트마다 타입이 달라질 수 있으므로 제네릭으로 사용

  - 타입 단언으로 일일히 설정해주면 다른 타입이 추가되어 사용될 때마다 다시 타입들을 정의해주어 사용해야 하는 번거로움이 생긴다.

<br />

#### 커스텀 훅의 흐름 파악

![Custom-Fetch-2](../src/images/custom-fetch-2.png)
...
- 삼항 연산자를 사용하여 `id` 값이 없을 경우 할일 목록 컴포넌트가 렌더링되고, `id` 값이 있을 경우 할일 상세 조회 컴포넌트가 렌더링된다.
...
... 
  ...
  const requestFetch = async (params: AxiosParams) => {
    try {
      setLoading(true);

      const response = await axios.get(params.url);
      const resData: ResData<T> = response.data;
