# `<head>`에 들어가는 태그
- 문서의 `메타데이터`는 웹사이트에 대한 정보를 포함하고 있다.
  - `검색엔진최적화(SEO)`를 위해 중요한 역할을 한다.
...
- 유니코드인 UTF-8의 charset(문자열 세트)으로 다양한 언어를 모두 표현할 수 있다.
```html
<meta charset="UTF-8" />
```
...
### `뷰포트(Viewport)`
- 사용자의 디바이스(veiwport) 크기에 맞게 컨텐츠 표기
...
```html
<!-- X-UA-Compatible : IE9 주소바에 호환성 버튼 안보이게, IE=edge : Edge 렌더링 엔진 사용 -->
