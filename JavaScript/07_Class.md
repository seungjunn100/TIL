...
- 인스턴스 레벨의 프로퍼티와 메서드
  ...
  <img src="../images/instance.png" alt="인스턴스 레벨의 프로퍼티와 메서드" />
...
- 클래스 레벨의 프로퍼티와 메서드
  ...
  <img src="../images/class.png" alt="클래스 레벨의 프로퍼티와 메서드" />
...
const student = new Student('짱구', '신');
console.log(student.firstName); // 짱구
console.log(student.fullName()); // 신 짱구, 함수로 호출


// 함수로 호출하는 방법이 싫다면 생성자 함수에서 미리 지정하면 되지 않을까? ❌
class Student {
  constructor (firstName, lastName) {
    this.firstName = firstName;
    this.lastName = lastName;
...
const student = new Student('짱구', '신');
student.firstName = '짱아';
console.log(student.firstName); // 짱아
console.log(student.fullName); // 신 짱구
// 외부에서 변경된 이름을 선언해도 이미 생성자 안에서 풀네임이 지정되어 업데이트 되지 않는다. 


// 그렇다면 함수로 호출하지 않고 속성처럼 호출하려면?
...
class Student {
  ...
  // 단순히 속성의 한 부분이라고 간주되는 것들을 함수로 만들어야할 때 get을 사용하면 프로퍼티를 호출하듯이 함수를 호출할 수 있다.
...
console.log(student.fullName); // 신짱구, 함수 호출, 접근을 할 때(get 함수 호출)
student.fullName = '흰둥이'; // 흰둥이, 할당을 할 때(set 함수 호출)
```

### 접근자 프로퍼티를 사용하는 경우
- 무언가를 처리해야하는 행동은 아닌데, 프로퍼티의 일부분을 조합해서 만들 때

- 특정한 일을 조금 더 해주고 싶을 때
  - 예를들어, 특정한 숫자를 `set`할 때, 0이하의 숫자가 들어오면 0으로 `set` 될 수 있도록 제어하고 싶을 때 사용
...
class Dog extends Animal {
  ...
  // 부모의 기능을 그대로 유지하면서 추가적인걸 하고 싶을 때
  eat() {
    super.eat();
    console.log('또 먹자~!');
  }
